...
util4G <- util4G %>%
  ...
  mutate(
    target = as.factor(ifelse(Payload >= q3Payload, "onTarget", "notOnTarget"))
  ) 
...
#predict rf
util4G.rf.pred <- predict(util4G.rf,train[,-9])

table("prediction"=util4G.rf.pred, "actual"=train$target)
confusionMatrix(table("prediction"=util4G.rf.pred, "actual"=train$target))
```

#Predict to test
```{r}
rf.pred.test <- predict(util4G.rf,test[,-9])

table("prediction"=rf.pred.test, "actual"=test$target)
confusionMatrix(table("prediction"=rf.pred.test, "actual"=test$target))
```
